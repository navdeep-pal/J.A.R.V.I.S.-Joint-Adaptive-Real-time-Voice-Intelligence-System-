...
def get_base_dir():
    # scripts/setup/bootstrap.py -> scripts/setup/ -> scripts/ -> root
...
def check_node(fix=False):
    ...
    try:
        ...
        if detected_major >= REQUIRED_NODE_MAJOR:
            ...
            print_error(f"Node.js v{REQUIRED_NODE_MAJOR}+ required. Detected: {detected_version}")
    ...
    print(f"\n{Colors.BOLD}Options:{Colors.ENDC}")
    print(f"[1] Download Node.js v{REQUIRED_NODE_MAJOR}+")
...
def check_and_update_hash(marker_path, source_file):
    ...
    """
    if not os.path.exists(source_file): return False, None
