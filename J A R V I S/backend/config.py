...
Singleton ConfigManager for J.A.R.V.I.S.
Reads and writes user_config.json, exporting typed accessors
for all API keys, user profile info, and LLM settings.
"""
import json
import os
import logging
...
class ConfigManager:
    ...
    def get_n8n_url(self) -> Optional[str]:
        """Helper to get n8n MCP server URL"""
        return self.get_api_key("n8n_url")

    def is_mds_enabled(self) -> bool:
        """Feature flag for MDS-driven adaptive behavior."""
        return bool(self.get("experiments.mds_enabled", True))

    def get_documents_dir(self) -> str:
        """Directory used for local document grounding."""
        project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
        default_path = os.path.join(project_root, "knowledge_base")
        configured = str(self.get("knowledge_base.documents_dir", default_path))
